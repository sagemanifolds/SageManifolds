...
$\mathbb{S}^n$, 3 charts for the real projective plane
...
A module with a basis is called a \emph{free module}. 
Moreover for a free module over a commutative ring, all bases have the same
cardinality, which is called the \emph{rank} of the module (or the \emph{dimension}
if the module is a vector space). 

Whenever $\mathcal{X}(U)$ is a free module, a basis is nothing but a \emph{vector frame}
...
For the implementation of vector fields in \SM{}, a key point is that 
...
$\mathcal{M}$ is parallelizable if, and only if, $\mathcal{X}(M)$
is a free module. Examples of parallelizable manifolds are \cite{Lee13}
\begin{itemize}
...
at least two and four charts respectively to cover them. 
...
The latter is a subclass of
\code{FiniteRankFreeModule}, which is the class devoted to generic free modules
of finite rank (cf. Fig.~\ref{f:module_classes}). 
Each vector field $\w{v}\in  \mathcal{X}(U_i)$ has different set
of components $(v^a)_{1\leq a\leq n}$ in different vector frames 
$(\w{e}_a)_{1\leq a \leq n}$ introduced on $U_i$. They are stored
...
$(\w{e}_a)$ exist\footnote{Using standard notation, in (\ref{e:T_expand})
