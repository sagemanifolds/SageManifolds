...
class VectorFrame(SageObject):
    ...
    def __init__(self, manifold, name, latex_name=None):
        ...
        self.manifold.frames[self.name] = self
        # The fist defined frame is considered as the default frame on the 
        # manifold:
        if self.manifold.def_frame is None: self.manifold.def_frame = self
        # Dual coframe:
        if self.name not in self.manifold.coframes:
            CoFrame(self.manifold, self.name, self.latex_name)
        self.coframe = self.manifold.coframes[self.name]
        ...
        for i in range(n):
            ...
            v = VectorField(manifold, v_name, v_symb)
            for j in range(n):
    ...
    def new_frame(self, change_of_basis, name, latex_name=None):
        r"""
        Set a new vector frame from the current one.
        ...
        the_new_frame = VectorFrame(self.manifold, name, latex_name)
        ...
        for i in range(si,nsi):
            for j in range(si,nsi):
                the_new_frame.vec[i-si].set_comp(self.name, 
                                                 delete_others=False)[[j]] = \
                                                  transf.comp(self.name)[[j,i]]
        # Components of the new coframe 1-forms in the old coframe: 
        for i in range(si,nsi):
            for j in range(si,nsi):
                the_new_frame.coframe.form[i-si].set_comp(self.name, 
                                                 delete_others=False)[[j]] = \
        ...
        for i in range(si,nsi):
            for j in range(si,nsi):
                transf.set_comp(name, delete_others=False)[[i,j]] = \
                                                  transf.comp(self.name)[[i,j]]
                inv_transf.set_comp(name, delete_others=False)[[i,j]] = \
                                              inv_transf.comp(self.name)[[i,j]]
        # Components of the old frame vectors in the new frame: 
        for i in range(si,nsi):
            for j in range(si,nsi):
                self.vec[i-si].set_comp(name, delete_others=False)[[j]] = \
                                                   inv_transf.comp(name)[[j,i]]
        # Components of the old coframe 1-forms in the new coframe: 
        for i in range(si,nsi):
            for j in range(si,nsi):
                self.coframe.form[i-si].set_comp(name, 
                                                 delete_others=False)[[j]] = \
                                                       transf.comp(name)[[i,j]]
        self.manifold.frame_changes[(self.name, name)] = transf
        self.manifold.frame_changes[(name, self.name)] = inv_transf
    ...
    def structure_coef(self):
        ...
        if self._structure_coef is None:
            self._structure_coef = CompWithSym(self.manifold, 3, self.name,
                                               antisym=(1,2))
            si = self.manifold.sindex
...
class CoordBasis(VectorFrame):
    ...
    sphere `S^2`::
        ...
        sage: m = Manifold(2, 'S^2', start_index=1)
        sage: Chart(m, r'th:[0,pi]:\theta ph:[0,2*pi):\phi', 'spher')
        chart 'spher' (th, ph)
    ...
    def __init__(self, chart):
        ...
        self.chart = chart
        name = chart.name + "_b"
        VectorFrame.__init__(self, chart.manifold, name)
...
class CoFrame(SageObject):
    ...
    - ``manifold`` -- the manifold on which the coframe is defined
    ...
    def __init__(self, manifold, name, latex_name=None):
        ...
        self.manifold.coframes[self.name] = self
        # Dual vector frame:
        if self.name not in self.manifold.frames:
            VectorFrame(self.manifold, self.name, self.latex_name)
        self.frame = self.manifold.frames[self.name]
        ...
        for i in range(n):
            ...
            v = OneForm(manifold, v_name, v_symb)
            for j in range(n):
                v.set_comp(self.name)[j+si] = 0
            v.set_comp(self.name)[i+si] = 1
            vl.append(v)
...
class CoordCoFrame(CoFrame):
    ...
    def __init__(self, chart, name=None, latex_name=None):
        ...
        if name is None:
            name = chart.name + "_b"
        CoFrame.__init__(self, chart.manifold, name, latex_name)
