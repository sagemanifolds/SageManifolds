...
class VectorField(TensorField) :
    ...
    - ``manifold`` -- the manifold on which the vector field is defined
    ...
    by the method :meth:`show`::
    
        sage: v.show()   # displays the expansion on the manifold's default frame (e)
        V = e_0 + 4 e_1 + 9 e_2
    ...
    def __call__(self, scalar):
        ...
        from scalarfield import ZeroScalarField, scalar_field
        ...
        if self.latex_name is not None and scalar.latex_name is not None:
            res_latex = self.latex_name + r"\left(" + scalar.latex_name + \
                        r"\right)"
        return scalar_field(res, name=res_name, latex_name=res_latex)
