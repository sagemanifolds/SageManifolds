...
class Chart(SageObject):
    ...
    - ``coordinates`` -- single string defining the coordinate symbols and 
      ...
      has at most three fields, separated by ':': 
        ...
        2. (optional) The interval defining the coordinate range: if not
    ...
    - ``name`` -- string containing the name given to the chart (should be 
      rather short and unique among all charts defined on the manifold)
    
    EXAMPLES: 
    
    Spherical coordinates on `\RR^3` (note the prefix 'r' for the string 
    ...
    Check that the declared coordinate ranges have been taken into account::
    ...
    """
    def __init__(self, manifold, coordinates, name): 
        ...
        if name not in manifold.atlas:
            self.name = name
        else:
            raise ValueError("The name '" + name + "' is already used for " + 
                             "another chart on " + str(manifold))
        ...
        glob = globals()  
        for i in range(n): 
            coord_properties = coord_list[i].split(':') # properties of coordinate no. i
        ...
        # The chart is added to the manifold's atlas:
    ...
    def coord_bounds(self, i=None):
        ...
        Some coordinate bounds on a 2-dimensional manifold::
            ...
            sage: m = Manifold(2, 'M')
            sage: c_xy = Chart(m, 'x y:[0,1)', 'xy-coords')
...
class MultiFunctionChart(SageObject):
    ...
    A set of 3 functions of 2 coordinates::
        ...
        sage: f
        functions (x - y, x*y, cos(x)*e^y) on the chart 'xy-coord' (x, y)
        sage: f.functions
        (x - y, x*y, cos(x)*e^y)
        sage: f(x,y)
        (x - y, x*y, cos(x)*e^y)
