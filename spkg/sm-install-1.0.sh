...
if [ "$1" != "no-download" ]; then
    echo -e "\n$(tput setaf 4)Downloading the sources of SageManifolds 1.0...$(tput sgr 0)"
    # Downloading with either wget or curl
    wget -N http://sagemanifolds.obspm.fr/spkg/manifolds-1.0.tar.gz || curl -O http://sagemanifolds.obspm.fr/spkg/manifolds-1.0.tar.gz

    if [ ! -f "manifolds-1.0.tar.gz" ]; then
...
./sage -b
echo -e "\n$(tput bold)$(tput setaf 4)Installation of SageManifolds 1.0 completed!\n$(tput sgr 0)"
