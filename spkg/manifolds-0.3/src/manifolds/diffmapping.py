...
EXAMPLES: 
    ...
    An example of diffeomorphism: a rotation in the Euclidean plane::
        ...
        sage: c_cart.<x,y> = m.chart('x y', 'cart') # Cartesian coordinates
...
class DiffMapping(SageObject):
    ...
    def __init__(self, domain1, domain2, coord_functions=None, 
        ...
        if coord_functions is not None:
            if chart1_name is None: chart1_name = domain1.def_chart.name
            if chart2_name is None: chart2_name = domain2.def_chart.name
            if chart1_name not in self.domain1.atlas:
                raise ValueError("The chart " + chart1_name +
                                    " has not been defined on the " + 
                                    str(self.domain1))
            if chart2_name not in self.domain2.atlas:
                raise ValueError("The chart " + chart2_name +
                                    " has not been defined on the " + 
                                    str(self.domain2))
            chart1 = self.domain1.atlas[chart1_name]
    ...
    def view(self, chart1_name=None, chart2_name=None):
        r""" 
        ...
        - ``chart1_name`` -- (default: None) name of the chart on the start
          domain; if none, the start domain's default chart will be used
        - ``chart2_name`` -- (default: None) name of the chart on the arrival
          domain; if none, the arrival domain's default chart will be used
        ...
        Standard embedding of the sphere `S^2` in `\RR^3`::
            ...
            sage: n = Manifold(3, 'R^3', r'\RR^3')
            sage: c_cart.<x,y,z> = n.chart('x y z', 'cart')
        ...
        if chart1_name is None:
            chart1_name = self.domain1.def_chart.name
        if chart2_name is None:
            chart2_name = self.domain2.def_chart.name
        expression = self.expr(chart1_name, chart2_name)
        chart1 = self.domain1.atlas[chart1_name]
        chart2 = self.domain2.atlas[chart2_name]
    ...
    def multi_function_chart(self, chart1_name=None, chart2_name=None):
        ...
        - ``chart1_name`` -- (default: None) name of the chart on the start
          domain; if None, the start domain's default chart is assumed
        - ``chart2_name`` -- (default: None) name of the chart on the arrival
          domain; if None, the arrival domain's default chart is assumed
        ...
        one::
            ...
            sage: c_xyz.<x,y,z> = n.chart('x y z', 'xyz')
            ...
            functions (u*v, u/v, u + v) on the chart 'uv' (M, (u, v))
        ...
        Representation in other charts::
            ...
            sage: c_UV.<U,V> = m.chart('U V', 'UV')  # new chart on M
            sage: ch_uv_UV = CoordChange(c_uv, c_UV, u-v, u+v)
            sage: ch_uv_UV.inverse()(U,V)
            (1/2*U + 1/2*V, -1/2*U + 1/2*V)
            sage: c_XYZ.<X,Y,Z> = n.chart('X Y Z', 'XYZ') # new chart on N
            sage: ch_xyz_XYZ = CoordChange(c_xyz, c_XYZ, 2*x-3*y+z, y+z-x, -x+2*y-z)
            sage: ch_xyz_XYZ.inverse()(X,Y,Z)
            (3*X + Y + 4*Z, 2*X + Y + 3*Z, X + Y + Z)
            sage: Phi.multi_function_chart('UV', 'xyz')
            functions (-1/4*U^2 + 1/4*V^2, -(U + V)/(U - V), V) on the chart 'UV' (M, (U, V))
            sage: Phi.multi_function_chart('uv', 'XYZ')
            functions (((2*u + 1)*v^2 + u*v - 3*u)/v, -((u - 1)*v^2 - u*v - u)/v, -((u + 1)*v^2 + u*v - 2*u)/v) on the chart 'uv'  (M, (u, v))
            sage: Phi.multi_function_chart('UV', 'XYZ')
            functions (-1/2*(U^3 - (U - 2)*V^2 + V^3 - (U^2 + 2*U + 6)*V - 6*U)/(U - V), 1/4*(U^3 - (U + 4)*V^2 + V^3 - (U^2 - 4*U + 4)*V - 4*U)/(U - V), 1/4*(U^3 - (U - 4)*V^2 + V^3 - (U^2 + 4*U + 8)*V - 8*U)/(U - V)) on the chart 'UV' (M, (U, V))

        """
        dom1 = self.domain1; dom2 = self.domain2
        def_chart1 = dom1.def_chart.name; def_chart2 = dom2.def_chart.name
        if chart1_name is None:
        ...
        if (chart1_name, chart2_name) not in self.coord_expression:
            ...
            if def_chart2 in change_arrival \
                    and (def_chart2, chart2_name) in dom2.coord_changes:
                sel_chart2 = def_chart2
            else:
                for ochart2 in change_arrival:
                    if (ochart2, chart2_name) in dom2.coord_changes:
                        sel_chart2 = ochart2
                        break 
            if sel_chart2 is not None:
                oexpr = self.coord_expression[(chart1_name, sel_chart2)]
                chg2 = dom2.coord_changes[(sel_chart2, chart2_name)]
                self.coord_expression[(chart1_name, chart2_name)] = \
            ...
            if def_chart1 in change_start \
                    and (chart1_name, def_chart1) in dom1.coord_changes:
                sel_chart1 = def_chart1
            else:
                for ochart1 in change_start:
                    if (chart1_name, ochart1) in dom1.coord_changes:
                        sel_chart1 = ochart1
                        break
            if sel_chart1 is not None:
                oexpr = self.coord_expression[(sel_chart1, chart2_name)]
                chg1 = dom1.coord_changes[(chart1_name, sel_chart1)]
                self.coord_expression[(chart1_name, chart2_name)] = \
            ...
            if (def_chart1, def_chart2) in self.coord_expression \
                ...
                for (ochart1, ochart2) in self.coord_expression:
                    if (chart1_name, ochart1) in dom1.coord_changes \
                        and (ochart2, chart2_name) in dom2.coord_changes:
                        sel_chart1 = ochart1
                        sel_chart2 = ochart2
                        break
    ...
    def expr(self, chart1_name=None, chart2_name=None):
        ...
        A rotation in some Euclidean plane::
            ...
            sage: c_spher.<r,ph> = m.chart(r'r:(0,+oo) ph:(0,2*pi):\phi', 'spher') # spherical coordinates on the plane
        ...
        Expression of the rotation in terms of Cartesian coordinates::
            ...
            sage: c_cart.<x,y> = m.chart('x y', 'cart') # Declaration of Cartesian coordinates
    ...
    def set_expr(self, chart1_name, chart2_name, coord_functions): 
        ...
        Cartesian coordinates::
            ...
            sage: c_cart.<x,y> = m.chart('x y', 'cart')  # Cartesian coordinates
            sage: c_spher.<r,ph> = m.chart(r'r:(0,+oo) ph:(0,2*pi):\phi', 'spher') # spherical coordinates
        ...
        the differentiable mapping::
            ...
            {('spher', 'spher'): functions (r, 1/3*pi + ph) on the chart 'spher' (R^2, (r, ph))}
        ...
        It is recovered by a call to :meth:`expr`::
            ...
            (-1/2*sqrt(3)*y + 1/2*x, 1/2*sqrt(3)*x + 1/2*y)
            sage: rot.coord_expression
            {('spher', 'spher'): functions (r, 1/3*pi + ph) on the chart 'spher' (R^2, (r, ph)), ('cart', 'cart'): functions (-1/2*sqrt(3)*y + 1/2*x, 1/2*sqrt(3)*x + 1/2*y) on the chart 'cart' (R^2, (x, y))}
    ...
    def add_expr(self, chart1_name, chart2_name, coord_functions): 
        r"""
        Set a new coordinate representation of the mapping.
        ...
        spherical-coordinate expression by hand::
            ...
            sage: rot.add_expr('spher', 'spher', (r, ph+pi/3))
    ...
    def __call__(self, p, chart1_name=None, chart2_name=None):
        ...
        if chart2_name is None: 
            chart2_name = self.domain2.def_chart.name
        if chart1_name is None: 
            def_chart1 = self.domain1.def_chart.name
        ...
        if self.domain2.manifold is RealLine:   # special case of a mapping to R
            ...
            return Point(self.domain2.manifold, y, chart2_name, name=res_name, 
    ...
    def pullback(self, tensor):
        ...
        Pullback on `S^2` of a scalar field defined on `R^3`::
            ...
            sage: c_spher.<th,ph> = m.chart(r'th:(0,pi):\theta ph:(0,2*pi):\phi', 'spher') # spherical coord. on S^2
        ...
        if ncov == 0:
            ...
            for chart2_name in tensor.express:
                for chart1_name in dom1.atlas:
                    if (chart1_name, chart2_name) in self.coord_expression:
                        phi = self.coord_expression[(chart1_name, chart2_name)]
                        coord1 = dom1.atlas[chart1_name].xx
                        ff = tensor.express[chart2_name]
                        resu.add_expr( ff(*(phi(*coord1))), chart1_name)
            return resu
        else:
            ...
            for frame2_name in tensor.components:
                frame2 = dom2.frames[frame2_name]
                if isinstance(frame2, CoordBasis):
                    chart2_name = frame2.chart.name
                    for chart1_name in dom1.atlas:
                        if (chart1_name, chart2_name) in self.coord_expression:
                            ...
                            for ind_new in ptcomp.non_redundant_index_generator(): 
                                ...
                                for ind_old in dom2.manifold.index_generator(ncov): 
                                    ...
                                    res += t
                                ptcomp[ind_new] = res
                            resu.components[frame1.name] = ptcomp
            return resu
...
class Diffeomorphism(DiffMapping):
    ...
    - ``chart1_name`` -- (default: None) string defining the chart in which the
      coordinates are given on domain1; if none is provided, the coordinates are
      assumed to refer to the domain's default chart
    - ``chart2_name`` -- (default: None) string defining the chart in which the
      coordinates are given on domain2; if none is provided, the coordinates are
      assumed to refer to the domain's default chart
    - ``name`` -- (default: None) name given to the differentiable mapping
    - ``latex_name`` -- (default: None) LaTeX symbol to denote the 
      differentiable mapping; if none is provided, the LaTeX symbol is set to 
    ...
    def __init__(self, domain1, domain2, coord_functions=None, 
                 chart1_name=None, chart2_name=None, name=None, 
                 latex_name=None): 
        DiffMapping.__init__(self, domain1, domain2, coord_functions, 
    ...
    def inverse(self, chart1_name=None, chart2_name=None): 
        ...
        - ``chart1_name`` -- (default: None) string defining the chart in which
          ...
          is provided, the default chart of the start domain will be used
        - ``chart2_name`` -- (default: None) string defining the chart in which
          the computation of the inverse is performed if necessary; if none 
          is provided, the default chart of the arrival domain will be used
        ...
        if chart1_name is None: chart1_name = self.domain1.def_chart.name
        if chart2_name is None: chart2_name = self.domain2.def_chart.name
