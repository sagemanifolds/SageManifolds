...
class TensorField(SageObject):
    ...
    def view(self, frame_name=None, chart_name=None):
        ...
        - ``frame_name`` -- (default: None) string containing the name of the 
          ...
          none is provided, the domain's default frame is assumed
        - ``chart_name`` -- (default: None) string containing the name of the 
          ...
          selected frame are expressed; if none is provided, the domain's 
        ...
        if chart_name is None:
            chart_name = frame.domain.def_chart.name
    ...
    def comp(self, frame_name=None, from_frame=None):
        ...
        - ``frame_name`` -- (default: None) string containing the name of the 
          ...
          the components are assumed to refer to the domain's default frame
        ...
        coordinates::
            ...
            sage: e_spher = m.frames['spher_b'].new_frame(change_basis, 'e_spher')
        ...
        if frame_name not in self.components:
            ...
            # Check whether frame_name corresponds to a subframe of a frame
            # where the components of self are known:
            for known_fname in self.components:
                if frame in manif.frames[known_fname].subframes:
                    new_comp = self._new_comp(frame)
                    old_comp = self.components[known_fname]
                    sdom = frame.domain
                    chart_name = sdom.def_chart.name
                    for ind in old_comp._comp:
                        new_comp._comp[ind] = ScalarField(sdom, \
                                    old_comp[[ind]].expr(chart_name), chart_name)
            ...
            if from_frame is None: 
                for known_frame in self.components:
                    if (known_frame, frame_name) in manif.frame_changes \
                            and (frame_name, known_frame) in manif.frame_changes:
                ...
                if from_frame is None:
                    ...
                    # is possible
                    for known_fname in self.components:
                        for sframe in manif.frames[known_fname].subframes:
                            sframe_name = sframe.name
                            if (sframe_name, frame_name) in manif.frame_changes \
                            and (frame_name, sframe_name) in manif.frame_changes:
                                from_frame = sframe_name
                ...
                if from_frame is None:
                    raise ValueError("No frame could be found for computing " + 
                                     "the components in frame " + frame_name)
    ...
    def set_comp(self, frame_name=None):
        ...
        """
        if frame_name is None: frame_name = self.domain.def_frame.name
        if frame_name not in self.components:
            if frame_name not in self.manifold.frames:
                ...
                raise ValueError("The vector frame " + frame_name +
                                 ...
                                 str(self.manifold))
            frame = self.manifold.frames[frame_name]
            self.components[frame_name] = self._new_comp(frame)
    ...
    def __call__(self, *args):
        ...
        def_frame_name = dom.def_frame.name
        if def_frame_name in self.components:
            frame_name = def_frame_name
            for arg in args:
                if def_frame_name not in arg.components:
                    frame_name = None
                    break
        if frame_name is None:
            # Search for another frame:
            for fr_name in self.components:
                frame_name = fr_name
                for arg in args:
                    if fr_name not in arg.components:
                        frame_name = None
                        break
        ...
        if frame_name is None:
            ...
            for fr_name in self.components:
                fr = manif.frames[fr_name]
                for sfr in fr.subframes:
                    sfr_name = sfr.name
                    frame_name = sfr_name
                    for arg in args:
                        if sfr_name not in arg.components:
                            frame_name = None
                            break
            ...
            if frame_name is not None:
                self.comp(frame_name) # components updated in the found subframe
        if frame_name is None:
            raise ValueError("No common frame for the components.")
        t = self.components[frame_name]
        v = [args[i].components[frame_name] for i in range(p)]
    ...
    def common_frame(self, other):
        ...
        - name of the common frame; if no common frame is found, None is 
          returned. 
        ...
        for frame1_name in self.components:
            if frame1_name in other.components:
                return frame1_name
        # 1b/ Search involving subframes
        dom2 = other.domain
        for frame1_name in self.components:
            frame1 = dom.frames[frame1_name]
            for frame2_name in other.components:
                frame2 = dom2.frames[frame2_name]
                if frame2 in frame1.subframes:
                    self.comp(frame2_name)
                    return frame2_name
                if frame1 in frame2.subframes:
                    other.comp(frame1_name)
                    return frame1_name
        #
    ...
    def common_coord_basis(self, other):
        r"""
        Find a common coordinate basis for the components of ``self`` and 
        ``other``. 
        ...
        - name of the common coordinate basis; if no common basis is found, 
          None is returned. 
        ...
        if def_frame in self.components and \
           ...
           isinstance(dom.def_frame, CoordBasis):
            return def_frame # the domain's default frame is privileged
        for frame1_name in self.components:
            if frame1_name in other.components and \
               isinstance(manif.frames[frame1_name], CoordBasis):
                return frame1_name
        # 1b/ Search involving subframes
        dom2 = other.domain
        for frame1_name in self.components:
            frame1 = dom.frames[frame1_name]
            if not isinstance(frame1, CoordBasis):
                continue
            for frame2_name in other.components:
                frame2 = dom2.frames[frame2_name]
                if not isinstance(frame2, CoordBasis):
                    continue
                if frame2 in frame1.subframes:
                    self.comp(frame2_name)
                    return frame2_name
        ...
        # If this point is reached, it is indeed necessary to perform at least 
        # one component transformation to get a common frame
        if isinstance(dom.def_frame, CoordBasis):
            if def_frame in self.components:
                for oframe in other.components:
                    if (oframe, def_frame) in dom.frame_changes:
                        other.comp(def_frame, from_frame=oframe)
        ...
        # via a single component transformation
        for sframe in self.components:
            if not instance(manif.frames[sframe], CoordBasis):
                continue
            for oframe in other.components:
                if not instance(manif.frames[oframe], CoordBasis):
                    continue
                if (oframe, sframe) in dom.frame_changes:
                    other.comp(sframe, from_frame=oframe)
                    return sframe
        ...
        for sframe in self.components:
            for oframe in other.components:
                if (sframe, def_frame) in dom.frame_changes and \
                   ...
                   isinstance(manif.frames[def_frame], CoordBasis):
                    self.comp(def_frame, from_frame=sframe)
                    other.comp(def_frame, from_frame=oframe)
                    return def_frame
                for frame in dom.frames:
                    if (sframe, frame) in dom.frame_changes and \
                       ...
                       isinstance(manif.frames[frame], CoordBasis):
                        self.comp(frame, from_frame=sframe)
                        other.comp(frame, from_frame=oframe)
                        return frame
        #
    ...
    def pick_a_frame(self):
        ...
        """
        if self.domain.def_frame.name in self.components:
            return self.domain.def_frame.name  # the default frame is privileged
        else:
    ...
    def pick_a_coord_basis(self):
        ...
        The coordinate basis associated to the domain's default chart is 
        privileged. 
        ...
        #    basis ?
        dom = self.domain
        def_coordb = dom.def_chart.name + "_b"
    ...
    def mtrace(self, lpos1, lpos2):
        ...
        cmp_res = self.components[frame_name].mtrace(lpos1, lpos2)
    ...
    def symmetrize(self, pos=None, frame_name=None):
        ...
        - ``frame_name`` -- (default: None) string containing the name of the 
          ...
          performed; if none, the domain's default frame will be used if
    ...
    def lie_der(self, vector):
        ...
        if id(vector) not in self._lie_derivatives:
            ...
            coord_basis_name = self.common_coord_basis(vector)
            ...
            resc = self._new_comp(manif.frames[coord_basis_name]) 
            si = manif.sindex
            nsi = manif.dim + si
            n_con = self.tensor_type[0]
