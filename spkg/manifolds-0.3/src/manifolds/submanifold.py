...
class Submanifold(Manifold):
    ...
    The sphere `S^2` as a submanifold of the Euclidean space `\RR^3`::
        ...
        sage: c_cart.<x,y,z> = M.chart('x y z', 'cart')   # Cartesian coordinates on R^3
        sage: S = Submanifold(M, 2, 'S^2', start_index=1) 
        sage: U = S.open_domain('U') # U = S minus two poles
        sage: c_spher.<th,ph> = U.chart(r'th:(0,pi):\theta ph:(0,2*pi):\phi', 'spher') # spherical coordinates on U
    ...
    A helix as a submanifold of `\RR^3`::
        ...
        sage: c_param.<t> = H.chart('t', 'param')
    ...
    Pullback of 1-forms defined on `\RR^3` to `S^2`::
        ...
        sage: dX = M.coframes['cart_b'] ; dX
        coordinate coframe 'cart_b' (dx,dy,dz)
        sage: dX(1)
    ...
    Pushforward of vector fields defined on `S^2` to `\RR^3`::
        ...
        sage: S.pushforward(e(1)).view('cart_b', 'spher')
        i_*(d/dth) = cos(ph)*cos(th) d/dx + cos(th)*sin(ph) d/dy - sin(th) d/dz
        sage: S.pushforward(e(2)).view('cart_b', 'spher')
    ...
    def plot(self, coord_ranges, chartname = None, **kwds):
        ...
        Plot of a torus embedded in `\RR^3`::
            ...
            sage: c_uv.<u,v> = W.chart(r'u:(0,2*pi) v:(0,2*pi)', 'uv') # cyclic coordinates on T
        ...
        Plot of an Archimedean spiral embedded in `\RR^2`::
            ...
            sage: c_param.<t> = S.chart('t', 'param')
        ...
        if self.ambient_manifold.name in ['R2', 'R^2', 'R3', 'R^3']:
            if 'cart' not in self.ambient_manifold.atlas:
                raise ValueError("For drawing, Cartesian coordinates must be "
                                 + "defined.")
            coord_functions = \
    ...
    def pushforward(self, tensor):
        ...
        - instance of :class:`TensorField` representing a field of fully 
          contravariant tensors of the ambiant manifold, field defined on 
        ...
        Pushforward of a vector field defined on `S^2`, submanifold of `\RR^3`::
            ...
            sage: iv.view('cart_b', 'spher') # the pushforward expanded on the Cartesian frame, with components expressed in terms of (th,ph) coordinates
            i_*(v) = -sin(ph)*sin(th) d/dx + cos(ph)*sin(th) d/dy
            
        The components of the pushforward vector are scalar fields on the submanifold::
        
            sage: iv.comp('cart_b')[[1]]
        ...
        # start domain for chart1_name
        chart1_name = None; chart2_name = None
        def_chart1 = dom1.def_chart ; def_chart1_name = def_chart1.name
        def_chart2 = self.ambient_manifold.def_chart 
        def_chart2_name = def_chart2.name
        if def_chart1.frame.name in tensor.components and \
               (def_chart1_name, def_chart2_name) in embed.coord_expression:
            chart1_name = def_chart1_name
            chart2_name = def_chart2_name
        else:
            for (chart1n, chart2n) in embed.coord_expression:
                if chart2n == def_chart2_name and \
                     dom1.atlas[chart1n].frame.name in tensor.components:
                    chart1_name = chart1n
                    chart2_name = def_chart2_name
                    break
        if chart2_name is None:
            ...
            for (chart1n, chart2n) in self.coord_expression:
                if dom1.atlas[chart1n].frame.name in tensor.components:
                    chart1_name = chart1n
                    chart2_name = chart2n
                    break
        ...
        chart2 = self.ambient_manifold.atlas[chart2_name]
        frame1 = chart1.frame
        frame2 = chart2.frame
        # Computation at the component level:
        tcomp = tensor.components[frame1.name]
        ...
        phi = embed.coord_expression[(chart1_name, chart2_name)]
        jacob = phi.jacobian()
        ...
        for ind_new in ptcomp.non_redundant_index_generator(): 
            ...
            for ind_old in self.index_generator(ncon): 
                t = tcomp[[ind_old]].function_chart(chart1_name)
                for i in range(ncon):
