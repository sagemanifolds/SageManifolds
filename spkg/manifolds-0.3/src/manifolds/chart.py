...
class Chart(SageObject):
    ...
    - ``name`` -- string containing the name given to the chart; it acts as a 
      label, and therefore should be rather short and must be unique among all 
      charts defined on the manifold
    ...
    Cartesian coordinates on `\RR^3`::
        ...
        sage: c_cart = Chart(M, 'x y z', 'cart') ; c_cart
        chart 'cart' (R^3, (x, y, z))
    ...
    construction::
        ...
        sage: M = Manifold(3, 'R^3', r'\RR^3', start_index=1)
        sage: c_cart.<x,y,z> = Chart(M, 'x y z', 'cart') ; c_cart
        chart 'cart' (R^3, (x, y, z))
    ...
    one may write::
        ...
        sage: M = Manifold(3, 'R^3', r'\RR^3', start_index=1)
        sage: c_cart.<x1,y1,z1> = Chart(M, 'x y z', 'cart') ; c_cart
        chart 'cart' (R^3, (x, y, z))
    ...
    coordinate symbol is quite convenient; so it is recommended to declare::
        ...
        sage: M = Manifold(3, 'R^3', r'\RR^3', start_index=1)
        sage: c_cart.<x,y,z> = Chart(M, 'x y z', 'cart')
    ...
    complement of the half-plane `\{y=0, x\geq 0\}`::
        ...
        sage: c_spher.<r,th,ph> = Chart(U, r'r:(0,+oo) th:(0,pi):\theta ph:(0,2*pi):\phi', 'spher') ; c_spher 
        chart 'spher' (U, (r, th, ph))
    ...
    Each constructed chart is automatically added to the manifold's atlas::
        ...
        sage: M.atlas
        {'spher': chart 'spher' (U, (r, th, ph)), 
         'cart': chart 'cart' (R^3, (x, y, z))}
    ...
    call operator, i.e. the operator ``()``::
        ...
        sage: c_cart(p)
        (1, 0, -2)
        sage: c_cart(p) == p.coord('cart')
        True
        sage: q = M.point((2,pi/2,pi/3), 'spher') # point defined by its spherical coordinates
        sage: c_spher(q)
        (2, 1/2*pi, 1/3*pi)
        sage: c_spher(q) == q.coord('spher')
        True
        sage: a = U.point((1,pi/2,pi)) # the default coordinates on U are the spherical ones
        sage: c_spher(a)
        (1, 1/2*pi, pi)
        sage: c_spher(a) == a.coord('spher')
        True
    ...
    we set::
        ...
        sage: c_cartU.<x,y,z> = Chart(U, 'x y z', 'cart_U') 
    ...
    """
    def __init__(self, domain, coordinates, name, names=None): 
        ...
        self.domain = domain
        if name not in self.manifold.atlas:
            self.name = name
        else:
            raise ValueError("The name '" + name + "' is already used for " + 
                             "another chart on " + str(self.manifold))
        ...
        for sd in self.domain.superdomains:
            sd.atlas[self.name] = self
            if sd.def_chart is None: 
                sd.def_chart = self
        # Construction of the coordinate basis associated to the chart:
        if self.domain.name != 'field R':      
    ...
    def _repr_(self):
        ...
        description += '(' + self.domain.name + ', ' + str(self.xx) + ')'
        return description
    ...
    def __call__(self, point):
        ...
        """
        return point.coord(chart_name=self.name)
    ...
    def coord_bounds(self, i=None):
        ...
        Some coordinate bounds on a 2-dimensional manifold::
            ...
            sage: m = Manifold(2, 'M')
            sage: c_xy.<x,y> = Chart(m, 'x y:[0,1)', 'xy-coords')
    ...
    def add_restrictions(self, restrictions):
        ...
        The restrictions are transmitted to subcharts::
            ...
            sage: X_A = X.subchart(A, 'Cart_A', x^2+y^2 > 1/4)
    ...
    def subchart(self, domain, name, restrictions):
        r"""
        ...
        - ``name`` -- string containing the name given to the subchart (should 
          be rather short and must be unique among all charts defined on the 
          manifold)
        ...
        of the global Cartesian coordinates::
            ...
            sage: c_cart.<x,y> = Chart(M, 'x y', 'xy') # Cartesian coordinates on R^2
            sage: D = M.open_domain('D') # the unit open disc
            sage: c_cart_D = c_cart.subchart(D, 'xy_D', x^2+y^2<1) 
        ...
        Cartesian coordinates on the annulus `1<\sqrt{x^2+y^2}<2`::
            ...
            sage: c_cart_A = c_cart.subchart(A, 'xy_A', [x^2+y^2>1, x^2+y^2<4])
            sage: p in A, q in A
            (False, False)
        ...
        res = Chart(domain, coordinates, name)
    ...
    def transition_map(self, other, transformations, intersection_name=None, 
                       chart1_name=None, restrictions1=None, chart2_name=None, 
                       restrictions2=None):
        r""" 
        ...
        - ``chart1_name`` -- (default: None) name to be given to the 
          restriction of the current chart to `U\cap V` if the latter differs
          from `U`
        - ``restrictions1`` -- (default: None) list of conditions on the 
          ...
          list [x>y]. 
        - ``chart2_name`` -- (default: None) name to be given to the 
          restriction of the other chart to `U\cap V` if the latter differs
          from `V`
        ...
        Transition map between two stereographic charts on the circle `S^1`::
            ...
            sage: cU.<x> = Chart(U, 'x', 'stereo_N') # Stereographic chart from the North pole
            sage: V = M.open_domain('V') # Complement of the South pole
            sage: cV.<y> = Chart(V, 'y', 'stereo_S') # Stereographic chart from the South pole
            sage: trans = cU.transition_map(cV, 1/x, 'W', 'stereo_N_W', x!=0, 'stereo_S_W', y!=0)
            sage: trans
            coordinate change from chart 'stereo_N_W' (W, (x,)) to chart 'stereo_S_W' (W, (y,))
            ...
            True
            sage: M.atlas
            {'stereo_S': chart 'stereo_S' (V, (y,)), 
             'stereo_N_W': chart 'stereo_N_W' (W, (x,)), 
             'stereo_S_W': chart 'stereo_S_W' (W, (y,)), 
             'stereo_N': chart 'stereo_N' (U, (x,))}

        Transition map between spherical chart and Cartesian chart on `\RR^2`::
        
            sage: M = Manifold(2, 'R^2')
            sage: c_cart.<x,y> = Chart(M, 'x y', 'cart')
            sage: U = M.open_domain('U') # the complement of the segment {y=0, x >= 0}
            sage: c_spher.<r,phi> = Chart(U, r'r:(0,+oo) phi:(0,2*pi):\phi', 'spher')
            sage: trans = c_spher.transition_map(c_cart, (r*cos(phi), r*sin(phi)), \
                                                 chart2_name='cart_U', restrictions2=(y!=0, x<0))
            sage: trans
            coordinate change from chart 'spher' (U, (r, phi)) to chart 'cart_U' (U, (x, y))
            ...
            sage: M.atlas # ...but a new chart has been created: 'cart_U'
            {'cart_U': chart 'cart_U' (U, (x, y)), 
             'spher': chart 'spher' (U, (r, phi)), 
             'cart': chart 'cart' (R^2, (x, y))}
        ...
        if dom is dom1:
            ...
            chart1 = self.subchart(dom, chart1_name, restrictions1)
        if dom is dom2:
            chart2 = other
        else:
            chart2 = other.subchart(dom, chart2_name, restrictions2)
...
class FunctionChart(SageObject):
    ...
    Function defined on a 2-dimensional chart::
        ...
        sage: m = Manifold(2, 'M')
        sage: c_xy.<x,y> = m.chart('x y', 'c_xy')
    ...
    def scalar_field(self, name=None, latex_name=None):
        ...
        result.express = {self.chart.name: self}
        result.coord_expression = {(self.chart.name, 'canonical'):
...
class MultiFunctionChart(SageObject):
    ...
    A set of 3 functions of 2 coordinates::
        ...
        sage: f
        functions (x - y, x*y, cos(x)*e^y) on the chart 'xy-coord' (M, (x, y))
        sage: f.functions
        (x - y, x*y, cos(x)*e^y)
        sage: f(x,y)
        (x - y, x*y, cos(x)*e^y)
...
class CoordChange(SageObject):
    ...
    def __init__(self, chart1, chart2, *transformations): 
        ...
        if chart1.domain == chart2.domain:
            ...
            for sdom in domain.superdomains:
                sdom.coord_changes[(chart1.name, chart2.name)] = self
            frame1 = chart1.frame.name
            frame2 = chart2.frame.name
            ch_basis = AutomorphismField(chart1.domain) 
            ch_basis.add_comp(frame1)[:, chart1.name] = self.jacobian
            ch_basis.add_comp(frame2)[:, chart1.name] = \
                                                                  self.jacobian
            for sdom in domain.superdomains:
                sdom.frame_changes[(chart2.frame.name, 
                                    chart1.frame.name)] = ch_basis
            if (chart1.frame.name, chart2.frame.name) not in \
                                                        domain.frame_changes:
                for sdom in domain.superdomains:
                    sdom.frame_changes[(chart1.frame.name, 
    ...
    def inverse(self):
        ...
        in the plane::
            ...
            {('xy-coord', 'uv-coord'): coordinate change from chart 'xy-coord' (M, (x, y)) to chart 'uv-coord' (M, (u, v))}                                                      
            sage: ch_to_xy = ch_to_uv.inverse() ; ch_to_xy
            coordinate change from chart 'uv-coord' (M, (u, v)) to chart 'xy-coord' (M, (x, y))
            sage: ch_to_xy.transf                                                         
            functions (1/2*sqrt(3)*v + 1/2*u, -1/2*sqrt(3)*u + 1/2*v) on the chart 'uv-coord'  (M, (u, v))
            sage: m.coord_changes
            {('xy-coord', 'uv-coord'): coordinate change from chart 'xy-coord' (M, (x, y)) to chart 'uv-coord' (M, (u, v)), ('uv-coord', 'xy-coord'): coordinate change from chart 'uv-coord' (M, (u, v)) to chart 'xy-coord' (M, (x, y))}
    ...
    def set_inverse(self, *transformations, **kwds):
        ...
        From Cartesian to spherical coordinates in the plane::
            ...
            sage: c_spher.<r,ph> = m.chart(r'r:(0,+oo) ph:(0,2*pi):\phi', 'spher')
            ...
            coordinate change from chart 'cart' (M, (x, y)) to chart 'spher' (M, (r, ph))
            sage: m.coord_changes
            {('cart', 'spher'): coordinate change from chart 'cart' (M, (x, y)) to chart 'spher' (M, (r, ph)), ('spher', 'cart'): coordinate change from chart 'spher' (M, (r, ph)) to chart 'cart' (M, (x, y))}
