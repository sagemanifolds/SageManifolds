...
class Metric(SymBilinFormField):
    ...
    Metric components in a frame different from the manifold's default one::
        ...
        coordinate change from chart 'uv' (M, (u, v)) to chart 'xy' (M, (x, y))
        sage: m.atlas
        {'xy': chart 'xy' (M, (x, y)), 'uv': chart 'uv' (M, (u, v))}
        sage: m.frames
    ...
    def set(self, symbiform):
        ...
        for frame_name in symbiform.components:
            self.components[frame_name] = symbiform.components[frame_name].copy()
    ...
    def inverse(self):
        ...
        for frame_name in self.components:
            if frame_name not in self._inverse.components:
                ...
                cinv = CompFullySym(dom.frames[frame_name], 2)
    ...
    def christoffel(self, chart_name=None):
        ...
        spherical coordinates::
            ...
            sage: X.<r,th,ph> = m.chart(r'r:[0,+oo) th:[0,pi]:\theta ph:[0,2*pi):\phi', 'spher')
            sage: g = Metric(m, 'g')
        ...
        if chart_name is None:
            frame_name = self.domain.def_chart.frame.name
        else:
            frame_name = self.domain.atlas[chart_name].frame.name
    ...
    def weyl(self, name=None, latex_name=None):
        ...
        manifold, for instance the hyperbolic space `H^3`::
            ...
            sage: X.<rh,th,ph> = m.chart(r'rh:[0,+oo):\rho th:[0,pi]:\theta  ph:[0,2*pi):\phi', 'rtp-coord')
            sage: g = Metric(m, 'g')
            sage: b = var('b')                                                        
    ...
    def determinant(self, frame_name=None):
        ...
        - ``frame_name`` -- (default: None) name of the vector frame with 
          ...
          if None, the domain's default frame is used. If a chart name is 
        ...
        if frame_name not in self._determinants:
            ...
            for chart_name in gg[[i1, i1]].express:
                ...
                resu.add_expr(detgm, chart_name=chart_name)
    ...
    def sqrt_abs_det(self, frame_name=None):
        ...
        if frame_name not in self._sqrt_abs_dets:
            ...
            for chart_name in detg.express:
                ...
                resu.add_expr(x, chart_name=chart_name)
    ...
    def volume_form(self, contra=0):
        ...
        if self._vol_forms == []:
            ...
            eps[[ind]] = self.sqrt_abs_det(dom.def_frame.name)
