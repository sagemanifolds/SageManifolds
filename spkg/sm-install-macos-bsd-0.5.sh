...
echo -e "\n$(tput setaf 4)Downloading the package SageManifolds 0.5...$(tput sgr 0)"
...
while (( ${#func[@]} > i )); do
	...
	gsed -i '/^'"${func[i]}"'/,/^def\|^cdef/{/^def\|^cdef/!s/^/#/g};/^'"${func[i]}"'/s/^/#/' src/sage/symbolic/pynac.pyx
	((i++))
done
...
echo -e "\n$(tput bold)$(tput setaf 4)Installation of SageManifolds 0.5 completed!$(tput sgr 0)"
