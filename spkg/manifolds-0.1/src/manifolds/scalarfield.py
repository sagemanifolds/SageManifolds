...
from chart import FunctionChart, ZeroFunctionChart
...
class ScalarField(DiffMapping, DiffForm):
    ...
    field::
        ...
        f :\ \left(\theta, \phi\right) \mapsto \sin\left(\theta\right) \cos\left(\phi\right)
    ...
    :class:`RealLineManifold`)::
        ...
        {('spher', 'canonical'): functions (sin(th)*cos(ph),) on the chart 'spher' (th, ph)}
    ...
    def function_chart(self, chart_name=None, from_chart=None):
        ...
        if chart_name not in self.express:
            ...
            new_expr = self.express[from_chart](*(change.transf.functions))
    ...
    def __pos__(self):
        ...
        for chart_name in self.express:
            result.express[chart_name] = + self.express[chart_name]
        if self.name is not None:
...
class ZeroScalarField(ScalarField):
    ...
    def __init__(self, manifold, name=None, latex_name=None):
        from manifold import RealLine
        DiffForm.__init__(self, manifold, 0, name, latex_name)
        # fields pertaining to DiffMapping:
        self.manifold1 = manifold
        self.manifold2 = RealLine
